...
\end{itemize}
\begin{obs}
    Aquest resultat confirma que el procés convergeix, de tal manera que la immersió resultant després de realitzar totes les etapes és isomètrica.
\end{obs}
{\color{green!50!black} \textit{Prova.} 
...
Per la Proposició \ref{prop:dos de tres}, sabem que la immersió resultant del procés és isomètrica. Ara bé, encara no tenim demostrat que aquesta immersió és de classe $C^1$. Per tal que sigui $C^1$, cal que la immersió sigui contínua i que les primeres derivades siguin contínues. Per assegurar que la immersió que obtenim al límit és contínua, només cal que la successió dels $B_3$ sigui convergent. De la mateixa manera, si la successió dels $B_2$ és convergent, aleshores les primeres derivades de la immersió resultant són contínues. Podem escollir, per tant, acotar el $B_2$ i $B_3$ al pas $r$ de l'etapa $s$ per la sèrie geomètrica

\begin{equation}
    B_2 \le \frac1{2^{(s+r)}} \quad \text{i} \quad B_{3} \le \frac1{2^{(s+r)}}.
\end{equation}
